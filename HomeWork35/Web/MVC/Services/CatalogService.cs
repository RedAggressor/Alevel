...
public class CatalogService : BaseService, ICatalogService
{
    ...
    {
        var dtoBrand = await ExecutSafeAsync(async () =>
            await _httpClient
            .SendAsync<ListModel<CatalogBrand>, object>(
                $"{_settings.Value.CatalogUrl}/GetListBrand",
                HttpMethod.Post,
                null)
        );

        var list = dtoBrand.List.Select(b => new SelectListItem
        {
            Value = $"{b.Id}",
            Text = $"{b.Brand}"
        });
